...
async function fetchUserPot() {
    ...
    try {
        const response = await fetch('https://3acb-2a00-23c8-3393-4f01-b53a-d3c1-f71-eaa7.ngrok-free.app/auth/user/funds', {
            method: 'GET',
            headers: {
...
async function handleDeposit(amount) {
    ...
    try {
        const response = await fetch('https://3acb-2a00-23c8-3393-4f01-b53a-d3c1-f71-eaa7.ngrok-free.app/auth/user/deposit', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,  // Send JWT token to backend for authentication
                'Content-Type': 'application/json'
...
async function fetchUserXP() {
    ...
    try {
        const response = await fetch('https://3acb-2a00-23c8-3393-4f01-b53a-d3c1-f71-eaa7.ngrok-free.app/auth/user/xp', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,  // Send JWT token to backend for authentication
                'Content-Type': 'application/json'
